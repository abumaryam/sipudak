...
<script>
    // Metode Perhitungan 
    <?php foreach ($titikhitung as $ttk) {
        $nilaiz = $ttk['zd'] / $ttk['satud'];
        $rmse = $ttk['jumlahkasus'] - $nilaiz;
        $powrmse = pow($rmse, 2);
        $data[strtoupper($ttk['nama'])] = round($nilaiz);
    } ?>;
    ...
    info.update = function(props) {
        this._div.innerHTML = '<h4>Daerah Rawan Kekerasan</h4>' + (props ?
            '<b>' + props.wiadkc + '</b> <br/>' +
            props.wiadkk + '' +
            '<br/>' + '<h6>Jumlah Kasus</h6>' + PERHITUNGAN[props.wiadkc] :
            '');
    };
    ...
    function style(feature) {
        return {
            ...
            dashArray: '3',
            fillOpacity: 0.7,
            fillColor: getColor(PERHITUNGAN[feature.properties.wiadkc])
        };
    }
